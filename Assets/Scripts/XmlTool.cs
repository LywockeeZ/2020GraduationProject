...
public static class XmlTool 
{
    ...
    public static StageMetaData LoadStageDataXml(string FileName , string StageName)
    {
        string path = Application.streamingAssetsPath + "/" + FileName + ".xml";
        if (File.Exists(path))
        {
            XmlDocument xmlDoc = new XmlDocument();
            xmlDoc.Load(path);
            XmlNode stage = xmlDoc.SelectSingleNode(StageName);
            XmlNodeList baseUnitData = stage.SelectSingleNode("底层类型").ChildNodes;
            XmlNodeList upperUnitData = stage.SelectSingleNode("上层类型").ChildNodes;

            int row = XmlConvert.ToInt32(stage.Attributes["Row"].Value);
            int column = XmlConvert.ToInt32(stage.Attributes["Column"].Value);
            ...
            return new StageMetaData(StageMetadata, row, column);
        }
        else
        {
            Debug.Log("数据文件不存在");
