...
{ 
    ...
    public class VertExmotionEditor : Editor {
        ...
        Vector2 m_brushMinMax = new Vector2(0f, 1f);
    ...
    /*
        ...
        public void DrawUI(VertExmotionBase vtm )
        {
            ...
            if (DrawSensorIcon(m_mode == eMode.SENSORS) && !warning)
            {
                ...
                {
                    Material[] sharedMaterials = vtm.renderer.sharedMaterials;
                    Shader shaderEditor = Shader.Find("Hidden/VertExmotion_editor");
                    for (int i = 0; i < sharedMaterials.Length; ++i)
                    {
                        if (sharedMaterials[i] == null)
                            continue;
                    ...
                    vtm.m_hideUnselectedMaterialID = false;
                }
#endif
		    ...
		    if( vtm.m_VertExmotionSensors.Count<VertExmotionBase.MAX_SENSOR )
		    {
                ...
                if (GUILayout.Button("Duplicate", EditorStyles.miniButton, GUILayout.Height(14)))
                {
...
#endif
                    {
                        ...
                        {
                            GameObject go = GameObject.Instantiate(vtm.m_VertExmotionSensors[m_sensorId].gameObject);
        ...
        void DrawBrushMenu()
	    {
            ...
            if (vtm.m_mesh.subMeshCount > 1)
            {
                ...
                if (m_selectedMaterialSlotID != 0)
                {
                    ...
                    vtm.m_hideUnselectedMaterialID = EditorGUILayout.ToggleLeft("Hide other materials", vtm.m_hideUnselectedMaterialID);
                }
                else
                {
            ...
            if (EditorGUI.EndChangeCheck())
            {
                ...
                if (vtm.m_hideUnselectedMaterialID && m_selectedMaterialSlotID != 0)
                {
                    Shader shaderHide = Shader.Find("Hidden/VertExmotion_hidden");
                    for (int i = 0; i < sharedMaterials.Length; ++i)
                    {
                        if (sharedMaterials[i] == null)
                            continue;
            ...
            GUILayout.Space(5);
            GUILayout.BeginHorizontal();
            GUILayout.Label ("Size : ", m_styleBold, GUILayout.ExpandWidth(true));
            GUILayout.FlexibleSpace();
            ...
            GUILayout.BeginHorizontal();
            GUILayout.Label ("Intensity : ", m_styleBold, GUILayout.ExpandWidth(true));
            GUILayout.FlexibleSpace();
            ...
            GUILayout.BeginHorizontal();
            GUILayout.Label("Falloff : ", m_styleBold, GUILayout.ExpandWidth(true));
            GUILayout.FlexibleSpace();
	    ...
	    void OnDisable()
	    {
           ...
           if (Application.isPlaying)
            ...
            for ( int i=0; i< sharedMaterials.Length; ++i )
		    {
			    ...
			    if(sharedMaterials[i].shader.name == "Hidden/VertExmotion_editor" )
                    sharedMaterials[i].shader = vtm.m_initialShaders[i];
